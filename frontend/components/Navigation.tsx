          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 border-t border-gray-200" style={{backgroundColor: '#66A5AD'}}>
            {filteredNavItems.map((item) => (
              <Link
                ...
                className="flex items-center px-3 py-2 rounded-md text-base font-medium transition-colors hover:opacity-80"
                style={{
                  backgroundColor: isActive(item.href) ? '#07575B' : 'transparent',
                  color: isActive(item.href) ? 'white' : '#003B46'
                }}
              >
                {item.name}
              </Link>
